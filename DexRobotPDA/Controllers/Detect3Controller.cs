...
public class Detect3Controller : ControllerBase
{
    private readonly DailyDbContext db;
    private readonly IMapper mapper;
    private readonly ILogger<Detect3Controller> _logger;

    public Detect3Controller(DailyDbContext _db, IMapper _mapper, ILogger<Detect3Controller> logger)
    {
        db = _db;
        mapper = _mapper;
        _logger = logger;
    ...
    [HttpGet]
    public async Task<IActionResult> GetDetect3(string split_id)
    {
        ...
        try
        {
            ...
            var latestDetect = await db.Detect3
            ...
            {
                // 映射为DTO返回
                var detectDto = mapper.Map<SplitCalibrateDetectDto>(latestDetect);
                response.ResultCode = 1;
    ...
    [HttpGet]
    public IActionResult GetSplitCalibrateDetectList(string task_id)
    {
        ApiResponse response = new ApiResponse();
        try
        {
            ...
            List<SplitCalibrateDetectDto> Detects = mapper.Map<List<SplitCalibrateDetectDto>>(list);
            response.ResultCode = 1;
            response.Msg = "Success";
    ...
    [HttpPost]
    public async Task<IActionResult> AddDetect3(SplitCalibrateDetectCreateDto addDetectDto)
    {
        ApiResponse response = new ApiResponse();
        try
        {
            ...
            bool motorExists = await db.Splits.AnyAsync(m => m.split_id == addDetectDto.split_id);
            if (!motorExists)
            {
                response.ResultCode = -1;
                response.Msg = $"分指机构ID '{addDetectDto.split_id}' 不存在，无法创建检测记录";
                _logger.LogWarning("新增检测记录失败：分指机构不存在 - {MotorId}", addDetectDto.split_id);
                return BadRequest(response);
            }

            // 3. 使用AutoMapper将DTO转换为实体
            var detectModel = mapper.Map<SplitCalibrateDetectModel>(addDetectDto);

            // 5. 添加到数据库并保存
            await db.Detect3.AddAsync(detectModel);
        ...
        catch (DbUpdateException dbEx)
        {
            ...
            _logger.LogError(dbEx, "新增检测记录时数据库操作失败，分指机构ID: {MotorId}", addDetectDto?.split_id);
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "新增检测记录时发生错误，分指机构ID: {MotorId}", addDetectDto?.split_id);
