...
public class MotorController : ControllerBase
{
    ...
    public async Task<IActionResult> GetMotor(string motor_id)
    {
        ...
        try
        {
            ...
            // 判断是否超过72小时，如果是则更新资格状态
            if (TimeHelper.GetHoursDifference(motors.created_at, DateTime.Now) >= 72)
            {
                motors.is_qualified = true;
                // 同时更新数据库中的实体
                motor.is_qualified = true;
    ...
    public async Task<IActionResult> GetMotorByFinger(string finger_id)
    {
        ...
        try
        {
            var motor = await db.Motors.FirstOrDefaultAsync(m => m.finger_id == finger_id);
            MotorDto motors = mapper.Map<MotorDto>(motor);
            response.ResultCode = 1;
            response.Msg = "Success";
            response.ResultData = motors;
