...
public class DetectService : BaseService
{
    ...
    public async Task<ApiResponse> AddDetect1(MotorDetectCreateDto detectCreateDto)
    {
        try
        {
            // 发送创建检测记录的请求
            var detectRequest = new RestRequest("api/Detect1/AddDetect1", Method.Post);
            detectRequest.AddJsonBody(detectCreateDto);

            _logger.LogInformation("开始为电机 {MotorId} 创建检测记录", detectCreateDto.motor_id);
            var detectResponse = await ExecuteCommand(detectRequest);

            if (detectResponse.ResultCode == 1)
            {
                _logger.LogInformation("电机 {MotorId} 的检测记录创建成功", detectCreateDto.motor_id);
            }
            else
            {
                _logger.LogWarning("电机 {MotorId} 的检测记录创建失败: {Message}", detectCreateDto.motor_id, detectResponse.Msg);
            }
            return detectResponse;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "创建电机 {MotorId} 的检测记录时发生异常", detectCreateDto.motor_id);
            throw;
        }
    }
    ...
    public async Task<ApiResponse> AddDetect2(SplitWormDetectCreateDto detect1Dto)
    {
        try
        {
            ...
            _logger.LogInformation("开始为分指机构 {MotorId} 创建检测记录", detect1Dto.finger_id);
            var detectResponse = await ExecuteCommand(detectRequest);

            if (detectResponse.ResultCode == 1)
            {
                _logger.LogInformation("分指机构 {MotorId} 的检测记录创建成功", detect1Dto.finger_id);
            }
            else
            {
                _logger.LogWarning("分指机构 {MotorId} 的检测记录创建失败: {Message}", detect1Dto.finger_id, detectResponse.Msg);
            }
            return detectResponse;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "创建分指机构 {MotorId} 的检测记录时发生异常", detect1Dto.finger_id);
            throw;
        }
    }
    ...
    public async Task<MotorDto> GetMotorByFinger(string finger_id)
    {
        var request = new RestRequest("api/Motor/GetMotorByFinger");
